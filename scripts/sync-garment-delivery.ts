...
function getCell(row: string[], indexMap: Map<string, number>, header: (typeof COLUMN_HEADERS)[number]): string | null {
...
function parseTabRows(tabName: string, rows: string[][]): ParsedRow[] {
  ...
  for (const row of rows) {
    ...
    const factoryRaw: string | null = getCell(row, headerMap, 'FACTORY');
...
export async function syncGarmentDelivery(): Promise<number> {
  ...
  // Truncate equivalent for PostgREST clients: delete all rows before full reload.
